...
from src.sortData import sort_ID_ascending
from src.sortData import sort_ID_descending
from src.sortData import sort_Name
from src.sortData import sort_Age
from src.sortData import sort_Gender
from src.sortData import sort_Class
...
def sort_by_id_ascending():
    global df
    arrange_df = sort_ID_ascending(df)
    if arrange_df is not None:
        df = arrange_df
        display_data_and_info(df, tree, info_text_widget)
def sort_by_id_descending():
    global df
    arrange_df = sort_ID_descending(df)
    if arrange_df is not None:
        df = arrange_df
        display_data_and_info(df, tree, info_text_widget)
def Sort_by_Name():
    global df
    arrange_df = sort_Name(df)
    if arrange_df is not None:
        df = arrange_df
        display_data_and_info(df, tree, info_text_widget)
def Sort_by_Age():
    global df
    arrange_df = sort_Age(df)
    if arrange_df is not None:
        df = arrange_df
        display_data_and_info(df, tree, info_text_widget)
def Sort_by_Gender():
    global df
    arrange_df = sort_Gender(df)
    if arrange_df is not None:
        df = arrange_df
        display_data_and_info(df, tree, info_text_widget)
def Sort_by_Class():
    global df
    arrange_df = sort_Class(df)
    if arrange_df is not None:
        df = arrange_df
        display_data_and_info(df, tree, info_text_widget)
...
name_sort_menu = Menu(sort_menu, tearoff=False)
sort_menu.add_cascade(label="Sắp xếp theo ID", menu=name_sort_menu)
name_sort_menu.add_command(label="Tăng dần", command=sort_by_id_ascending)
name_sort_menu.add_command(label="Giảm dần", command=sort_by_id_descending)
sort_menu.add_cascade(label="Sắp xếp theo Tên", command=Sort_by_Name)
sort_menu.add_cascade(label="Sắp xếp theo Tuổi", command=Sort_by_Age)
sort_menu.add_cascade(label="Sắp xếp theo Giới tính", command=Sort_by_Gender)
sort_menu.add_cascade(label="Sắp xếp theo Lớp", command=Sort_by_Class)
